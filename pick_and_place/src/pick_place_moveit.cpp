...
bool place(planning_scene_manager::PlanningSceneManager scene_mgr,
  move_group_manager::MoveGroupManager& group_mgr, geometry_msgs::PointStamped placing_point, moveit_msgs::DisplayTrajectory display_trajectory, ros::Publisher display_publisher)
{
...
bool detect_grasps2Cams(const Eigen::Vector3d& view_point, const Eigen::Vector3d& view_point2, const pcl::PointCloud<pcl::PointXYZRGB> cloud,
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber p_sub  = node_handle.subscribe<geometry_msgs::PointStamped>("placing_pose_pub", 1,  boost::bind(pCallback, _1, boost::ref(placing_point)));
  ...
  planning_scene_monitor::PlanningSceneMonitorPtr psm_ptr = planning_scene_monitor::PlanningSceneMonitorPtr( new planning_scene_monitor::PlanningSceneMonitor("robot_description"));
  boost::shared_ptr<moveit::planning_interface::MoveGroup> arm_group(new moveit::planning_interface::MoveGroup ("arms"));
  ...
  while (ros::ok())
  {
    ...
    if (grasp_cmd_recv)
    {
      ...
      if (use_gpd)
      {
        ...
        if (!detect_grasps2Cams(transformPR2.translation(),transformCam2.translation(), cloud, grasp_detector, grasps))
    ...
    if (place_cmd_recv)
    {
      ...
      if (placing_point_transformed.point.z<table.primitive_poses.at(0).position.z && placing_point_transformed.point.z>0.9){//if point is under the table
        goal_reached_pub.publish(msg1);
        continue;
      }
      ...
      cout<<placing_point_transformed.point.x<<", "<<placing_point_transformed.point.y<<", "<<placing_point_transformed.point.z<<", "<<endl;
      ...
      if (placing_point_transformed.point.x>x1 && placing_point_transformed.point.x<x2)
      {
          if (placing_point_transformed.point.y>y1 && placing_point_transformed.point.y<y2)
          {
              ...
              bool success = place(scene_mgr, group_mgr, placing_point_transformed, display_trajectory, display_publisher);
              psm_ptr->requestPlanningSceneState();
